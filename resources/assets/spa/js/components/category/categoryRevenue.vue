<template>
	<div>
		<category-tree :categories="categories" namespace="categoryRevenue"></category-tree>
		<category-register 
		...
		<modal :modal="modalDeleteOptions">
			...
			<div slot="footer">
				<button class="btn btn-flat waves-effect green lighten-2 modal-action modal-close" @click.prevent="destroy()">OK</button>
...
<script type="text/javascript">
	...
	export default {
		...
		created(){
			EventHub.$on('categoryRevenueNew', this.modalNew);
			EventHub.$on('categoryRevenueEdit', this.modalEdit);
			EventHub.$on('categoryRevenueDelete', this.modalDelete);
			EventHub.$on('categoryRevenueSave', this.saveCategory);
		}
	}
</script>